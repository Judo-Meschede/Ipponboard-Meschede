...
async function saveForm(){const old=(data[currentTab]||[]).find(x=>x.id===selectedId)||{};const rec={...old};$$('#detailBody [data-field]').forEach(el=>{rec[el.dataset.field]=el.type==='checkbox'?el.checked:(el.dataset.unit==='minutes'?(el.value===''?'':Math.round(Number(el.value)*60)):(el.type==='number'?(el.value===''?'':Number(el.value)):el.value))});if(currentTab==='teams')rec.fighterIds=$$('#detailBody [data-roster]:checked').map(x=>x.dataset.roster);if(currentTab==='competitionDays')rec.teamIds=$$('#detailBody [data-team]:checked').map(x=>x.dataset.team);const lp=$('#logoPreview');if(currentTab==='clubs'&&lp?.dataset.newLogo)rec.logo=lp.dataset.newLogo;try{await api('/api/masterdata/'+currentTab,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(rec)});toast('Gespeichert');selectedId=null;await load()}catch(e){toast('Fehler: '+e.message)}}
